"""
Script to predict stock prices using Linear Regression.
...
from ml.linear_reg.fetch_data import fetch_stock_data, add_technical_indicators
...
def tune_hyperparameters(X_train, y_train, cv=3, random_state=42):
    """
...
def get_feature_coefficients(model, feature_cols, top_n=20):
    """
    Get and display feature coefficients from the trained model.
    ...
    Returns:
        pd.DataFrame: DataFrame with feature coefficients
    """
    ...
    print("=" * 70)
    for idx, row in feature_coef_df.head(top_n).iterrows():
        print(f"{row['Feature']:40s} {row['Coefficient']:+.6f}")

    return feature_coef_df


def train_linear_regression_model(df, target_col='Close', test_size=0.2, random_state=42,
                                    tune_params=False, use_ridge=True):
    """
    ...
    Args:
        ...
        random_state (int): Random state for reproducibility
        tune_params (bool): Whether to perform hyperparameter tuning (for Ridge)
        use_ridge (bool): Whether to use Ridge regression (with regularization)
    ...
    if use_ridge:
        if tune_params:
            ...
            best_params = tune_hyperparameters(X_train_scaled, y_train, cv=3, random_state=random_state)
            alpha = best_params['alpha']
        else:
            # Use optimized alpha value
            alpha = 1.0
    ...
    get_feature_coefficients(model, feature_cols, top_n=20)
...
def predict_stock(ticker, days_ahead=30, period="2y", plot=True):
    """
    ...
    """
    print(f"Predicting {ticker} stock prices for the next {days_ahead} days using Linear Regression")
    print("="*60)
    ...
    print(f"Prepared features. Final shape: {df_features.shape}")

    # Step 4: Train model
    print("\nStep 4: Training Linear Regression model...")
    model, scaler, feature_cols, X_test, y_test, test_predictions = train_linear_regression_model(df_features)
    ...
    # Display predictions
    print(f"\n{'='*60}")
    print(f"Future Predictions for {ticker} (Next {days_ahead} Days):")
    print(f"{'='*60}")
...
def predict_stock_for_api_with_pretrained(ticker, days_ahead=7, use_pretrained=True):
    ...
    model_result = load_or_train_model(
        ...
        train_function=lambda df: train_linear_regression_model(df),
    ...
    # We need to get feature_cols from somewhere
    ...
    result = {
        'ticker': ticker.upper(),
        'model_name': 'Linear Regression (Pre-trained)' if not model_result.get('trained_now') else 'Linear Regression',
...
def predict_stock_for_api(ticker, days_ahead=7, period="2y"):
    """
    ...
    # Train model
    train_result = train_linear_regression_model(df_features)
